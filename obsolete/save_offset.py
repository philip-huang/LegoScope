...
def compute_offset(camera, model, fx = 1100 , fy = 1100, z = 30.0, show_yolo = False, visualize = False):
    ...
    segment_top = np.zeros(og_frame.shape[:2])
    centers = []
    radiuses = []
    ...
    if visualize:
        ...
        segment_top=cv2.fillPoly(segments, [bottom_stud_mask.astype(np.int32)], 255)
    ...
    output[1] *= -0.7 
    output[2] *= 0.5
    return og_frame, segment_top, bottom_stud_center_adj  #mm to meter

def create_new_set_folder(base_path="data"):
    os.makedirs(base_path, exist_ok=True)
    i = 0
    while os.path.exists(os.path.join(base_path, f"set-{i}")):
        i += 1
...
if __name__ == "__main__":
    ...
    set_path = create_new_set_folder()
    labels_file_path = os.path.join(set_path, "labels.txt")
    ...
    while True:
        ...
        center = None
        try:
            og_frame, segment, center= compute_offset(camera, model, show_yolo=False, visualize=True)
        except:
            pass
        if center is not None:
            # Save segment as j.png where j is the frame number
            segment_path = os.path.join(set_path, f"{frame_count}.png")
            cv2.imwrite(segment_path, segment)
            og_path = os.path.join(set_path, f"og{frame_count}.png")
            cv2.imwrite(og_path, og_frame)
            # Write target center adj and angle to labels.txt
