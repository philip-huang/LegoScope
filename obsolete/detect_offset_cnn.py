...
from train_circle_cnn import CircleCNN
...
def compute_offset(camera, model, circle_model, fx = 1100 , fy = 1100, z = 30.0, show_yolo = False, visualize = False):
    '''
    Arguments: 
    ...
    top_segment = np.zeros(og_frame.shape[:2]).astype(np.uint8)
    bottom_segment = np.zeros(og_frame.shape[:2]).astype(np.uint8)
    centers = []
    radiuses = []
    ...
    top_segment=cv2.fillPoly(top_segment, [top_stud_mask.astype(np.int32)], 255)
    bottom_segment=cv2.fillPoly(bottom_segment, [bottom_stud_mask.astype(np.int32)], 255)
    top_stud_center = circle_model.predict(top_segment)
    bottom_stud_center = circle_model.predict(bottom_segment)
    ...
    if visualize:
        ...
        cv2.circle(og_frame, top_stud_center,top_stud_radius_adj, [230,230,0], 2)
        cv2.circle(og_frame, bottom_stud_center,bottom_stud_radius_adj, [230,230,0], 2)
...
if __name__ == "__main__":
    ...
    circle_model = CircleCNN().to(device)
    circle_model.load_state_dict(torch.load(model_path, map_location=device))
    circle_model.eval()
    # Set the loop rate (e.g., 10 Hz)
    while True:
        # Get the detected offset
        offset = compute_offset(camera, model, circle_model, show_yolo=False, visualize=True)
        print(offset)
